...
class rooms():
    ...
    def check_neighbour_rooms(self, player_location, room_type):
        for i in player_location:
            if i in self.rooms_dictionary.get(room_type):
                return True
        return False
        
    
    def check_neighbour_teleport_rooms(self, player_location):
        if player_location in self.rooms_dictionary.get("teleport_rooms"):
            return True
        return False

    def check_neighbour_wumpus_room(self, player_location):
        if player_location in self.rooms_dictionary.get("wumpus_room"):
            return True
        return False
    ...
    while player_one.is_alive():
        while wumpus_room_one.is_boss_alive():
            ...
            if x == "m":
                ...
                if room_one.check_neighbour_rooms(list_player_neighbour, "danger_rooms"):
                    """checks the surrounding for death"""
                    print("You can feel the wind gust")
                elif room_one.check_neighbour_rooms(list_player_neighbour, "wumpus_room"):
                    """checks the surrounding for wumpus"""
                    print("You can smell wumpus")
                elif room_one.check_neighbour_rooms(list_player_neighbour, "teleport_rooms"):
                    """checks the surrounding for teleport"""
                    print("You can hear bats")
                    print(list_teleport)
                elif room_one.check_neighbour_rooms(list_player, "teleport_rooms"):
                    print("You got teleported")
                    i = random.randint(0, 9)    
                    lst = list_safe[i]
                    player_one.start_coordinate(lst)                   
                elif room_one.check_neighbour_rooms(list_player, "danger_rooms"):
